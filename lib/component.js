...
export class Component {
  ...
  update(data) {
    ...
    this.element = patch(this.oldNode, this.element)
    if (!this.mounted) {
      this.componentWillMount && this.componentWillMount(this)
      if (!this.container || this.container.nodeType !== 1) {
        console.error(
          'The container for a class comonent is not a valid DOM node. Check the selector provided for the class to make sure it is a valid CSS selector and that the container exists in the DOM. You might be targeting a nonexistent node.'
        )
      }
